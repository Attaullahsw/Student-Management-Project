...
if (isset($_POST['month'])) {
    ...
    foreach ($allStudent as $key) {
        ...
        if ($count > 0) {
            $feedetailsDues = $db->fetch_multi_row('fee_tbl', array('fee_id', 'fee_amount', 'status', 'fee_date', 'fee_sumit_date', 'fine_title', 'fine_amount'), array('student_id' => $key->student_id, 'month' => $month));
            foreach ($feedetailsDues as $feeKey) {
                ...
                if ($feeKey->status == 0) {
                    ...
                    $outPut = $outPut . "<td>$feeKey->fee_date</td><td>Null</td>";
                    ...
                    $outPut = $outPut . "<td>$feeKey->fee_date</td><td>$feeKey->fee_sumit_date</td>";
                    $outPut = $outPut . '<td><span class="label-default label label-success">PAID</span></td>';
                }
                $outPut = $outPut . '</tr>';
            }
            $i++;
        } else {
            $outPut = $outPut . '<td>NULL</td><td>NULL</td><td>NULL</td><td><span class="label-default label label-danger">Not issued yet!</span></td></tr>';
            $i++;
        }
    }
